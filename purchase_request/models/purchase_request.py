...
class PurchaseRequest(models.Model):
    ...
    agreement_count = fields.Integer(
        string="Agreement count", compute="_compute_agreements_count", readonly=True
    )
    attachment_ids = fields.Many2many(
        'ir.attachment', 'pr_attachment_rel',
        'purchase_request_id', 'att_id',
        string='Attachments',
    ...
    def _compute_agreements_count(self):
        self.agreement_count = len(self.mapped("line_ids.requisition_lines.requisition_id"))


    @api.depends("line_ids", "line_ids.estimated_cost","line_ids.product_qty")
    def _compute_estimated_cost(self):
        for rec in self:
            rec.estimated_cost = sum(line.estimated_cost * line.product_qty for line in rec.line_ids)
    ...
    def action_view_requisition_order(self):
        ...
        lines = self.mapped("line_ids.requisition_lines.requisition_id")
        if len(lines) > 1:
            action["domain"] = [("id", "in", lines.ids)]
        elif lines:
            action["views"] = [
    ...
    def button_budget_approve(self):
        return self.write({"state": "budget_approve"})
    ...
    def button_canceled(self):
        self.mapped("line_ids").do_cancel()
        for po in self.mapped("line_ids.purchase_lines.order_id"):
            po.button_cancel()
        for pa in self.mapped("line_ids.requisition_lines.requisition_id"):
            pa.action_cancel()
